...
dat.bio.df <- as.data.frame(dat.bio)[tr$tip.label, ]
...
dat.class.mrca <- data.frame(
  ...
  node = sapply(unique(dat.class$SUBGENUS), function(i) {
    findMRCA(tr, row.names(dat.class)[dat.class$SUBGENUS == i])
  })
  )
dat.class.mrca <-
  dat.class.mrca[dat.class.mrca$Subgenus != 'unplaced', ]

cndata <- read.csv("./DATA/PPAdata.csv")    
cn <- as.numeric(cndata$C2n)
names(cn) <- cndata$species
cn
